...
export function part1() {
  ...
  const sphereImageurl = textureLoader.load(sphereImage)
  const sphereMetalImageurl = textureLoader.load(sphereMetalImage)
  
  const material1 = new THREE.MeshBasicMaterial( { color: 0xfff000 } );
  const mesh = new THREE.Mesh( geometry, material1 );

  //const material2 = new THREE.MeshBasicMaterial({ map: sphereImageurl })

//   const material2 = new THREE.MeshNormalMaterial()
//   material2.flatShading = true
  // const material2 = new THREE.MeshMatcapMaterial({ matcap: sphereImageurl })  
...
//   ambentLight.position.set(2, 3, 4)
